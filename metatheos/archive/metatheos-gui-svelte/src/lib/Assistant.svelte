  ...
  onMount(async () => {
    try {
      ...
      if (!hasApiKey) {
        ...
        toastMessage =
          "No API key detected. You can use the webview chatbot or configure ANTHROPIC_API_KEY.";
        toastType = "info";
        toastShow = true;
        activeMode = "webview";
      }
...
<div class="h-full flex flex-col">
    ...
    <div class="flex items-center gap-2">
      {#if !loading}
        ...
        <div class="flex gap-1 p-1 bg-gray-100 dark:bg-gray-800 rounded-lg">
          <button
            class="px-3 py-1 text-sm rounded transition-colors {activeMode ===
            'ollama'
        ...
        {#if messages.length > 0 && activeMode === "claude"}
          <button class="btn btn-secondary text-sm" on:click={clearChat}
            >Clear Chat</button
          >
        {/if}
      {/if}
  ...
  {:else if activeMode === "ollama"}
    <div class="flex-1 flex flex-col gap-3 overflow-y-auto">
      <div
        class="card bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700"
      >
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-semibold text-gray-900 dark:text-white">
              Local model (Ollama) — Reason-only — No auto writes
            </p>
            <p class="text-xs text-gray-600 dark:text-gray-400">
              Base URL: {reasonerResult?.base_url || "http://127.0.0.1:11435"} ·
              Model: {reasonerResult?.model || "qwen2.5:7b-instruct"}
            </p>
          </div>
          <span class="text-xs px-2 py-1 rounded bg-orange-100 text-orange-700"
            >No vault writes by model</span
          >
        </div>
      </div>
      ...
      <div class="card">
        <div class="flex flex-col gap-2">
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400">
            Ollama reasons; Metatheos validates and writes via Safe Write.
          </p>
        </div>
      </div>
      ...
      {#if reasonerResult}
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div class="flex items-center justify-between mb-2">
              ...
              <div class="flex gap-2">
                ...
                <span
                  ...
                  >No write performed</span
                >
              </div>
            </div>
            <p class="text-sm text-gray-700 dark:text-gray-300 mb-2">
              Intent: {reasonerResult.intent} (confidence {Math.round(
                reasonerResult.confidence * 100,
              )}%) · {reasonerResult.rationale}
            </p>
            {#if reasonerResult.required_inputs?.length}
              <p class="text-xs text-gray-500 dark:text-gray-400 mb-2">
                Required inputs: {reasonerResult.required_inputs.join(", ")}
              </p>
            {/if}
            <div class="mt-3">
              ...
              <div class="mt-2 space-y-1 text-sm">
                {#if reasonerResult.context_used.phase}
                  <div>
                    <strong>Phase:</strong>
                    {reasonerResult.context_used.phase.id} ({reasonerResult
                      .context_used.phase.status || "n/a"})
                  </div>
                {/if}
                {#if reasonerResult.context_used.goals.length}
                  <div>
                    <strong>Goals:</strong>
                    {reasonerResult.context_used.goals
                      .map((g) => g.id)
                      .join(", ")}
                  </div>
                {/if}
                {#if reasonerResult.context_used.decisions.length}
                  <div>
                    ...
                    {reasonerResult.context_used.decisions
                      .map((g) => g.id)
                      .join(", ")}
                  </div>
                {/if}
                {#if reasonerResult.context_used.audits.length}
                  <div>
                    ...
                    {reasonerResult.context_used.audits
                      .map((g) => g.id)
                      .join(", ")}
                  </div>
                {/if}
                {#if reasonerResult.context_used.daily_notes.length}
                  <div>
                    <strong>Daily:</strong>
                    {reasonerResult.context_used.daily_notes
                      .map((g) => g.id)
                      .join(", ")}
                  </div>
                {/if}
          ...
          <div class="card">
            ...
            {#if reasonerResult.validation.warnings.length}
              <ul
                class="mt-2 text-sm text-yellow-700 dark:text-yellow-300 list-disc list-inside"
              >
        ...
        <div class="card">
          <div class="flex items-center justify-between mb-2">
            ...
            <span class="text-xs text-gray-500"
              >Path: {reasonerResult.draft_path || "n/a"}</span
            >
          </div>
          {#if reasonerResult.draft_markdown}
            ...
            <div class="flex flex-wrap gap-2 mt-3 items-center">
              <button
                ...
                on:click={() =>
                  (draftEditor = reasonerResult.draft_markdown || "")}
                disabled={!reasonerResult.draft_markdown}
              >
                Reset Draft
              </button>
              <button
                class="btn btn-primary"
                on:click={safeWriteDraft}
                disabled={!reasonerResult.validation.valid ||
                  sending ||
                  !reasonerResult.draft_path}
              >
                Apply via Safe Edit
              </button>
              <input
              ...
              <span class="text-xs text-gray-500"
                >No write performed until you apply.</span
              >
            </div>
          {:else}
            <p class="text-sm text-gray-600 dark:text-gray-300">
              No draft produced (analysis-only intent). Nothing to apply.
            </p>
          {/if}
        </div>
        ...
        <div class="card">
          ...
          <pre
            class="bg-gray-100 dark:bg-gray-900 text-xs p-3 rounded overflow-auto">{reasonerResult.raw_model_output}</pre>
        </div>
      {/if}
    </div>
  {:else if activeMode === "webview"}
    ...
    <div class="flex-1 card overflow-hidden">
      ...
      <div
        class="mt-2 p-3 bg-blue-50 dark:bg-blue-900/20 rounded border border-blue-200 dark:border-blue-800"
      >
        <p class="text-sm text-blue-800 dark:text-blue-200">
          <strong>ℹ️ Web Chatbot Mode (NO VAULT CONTEXT):</strong> Using
          claude.ai web interface. For integrated AI assistance with your
          governance data, set the
  ...
  {:else}
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
      {#if lastContextDescriptor}
        <div
          class="card mb-3 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700"
        >
          <div class="flex items-center justify-between">
            <h3 class="font-semibold text-gray-900 dark:text-white">
              Context Used
            ...
            <span
              class="text-xs px-2 py-1 rounded bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200"
            >
          ...
          <div class="mt-2 grid grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <p class="font-mono text-gray-900 dark:text-white">
                {lastContextDescriptor.phase || "n/a"}
              </p>
            </div>
            <div>
            ...
            <div>
              ...
              <p class="text-gray-900 dark:text-white truncate">
                {lastContextDescriptor.included_goals &&
                lastContextDescriptor.included_goals.length
      ...
      <!-- Chat Messages -->
      <div
        class="flex-1 overflow-y-auto chat-messages space-y-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg"
      >
        {#if !hasApiKey}
          <div
            class="card bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800"
          >
            <h3 class="font-semibold text-yellow-900 dark:text-yellow-100 mb-2">
              ⚠️ No API Key Configured
            </h3>
            <p class="text-sm text-yellow-800 dark:text-yellow-200 mb-2">
              To use AI assistance, please set your <code
                class="px-1 py-0.5 bg-yellow-100 dark:bg-yellow-900 rounded"
                >ANTHROPIC_API_KEY</code
              > environment variable.
            </p>
            <p class="text-sm text-yellow-700 dark:text-yellow-300">
              Alternatively, use the <strong>Web Chatbot</strong> tab above to access
              Claude directly through your browser.
            </p>
          </div>
        {/if}
          ...
          >
            <div class="max-w-[80%]">
              ...
              >
                ...
                {#if message.rejected}
                  <div
                    class="text-sm whitespace-pre-wrap text-red-600 dark:text-red-300"
                  >
                    {message.text}
                  </div>
                  {#if message.unknown?.length}
                    <p class="text-xs text-red-500 dark:text-red-300 mt-2">
                      Unknown references: {message.unknown.join(", ")}
                    </p>
                  {/if}
                {:else}
        ...
        {#if sending}
          <div class="flex gap-3 justify-start">
            <div
              class="px-4 py-3 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700"
            >
              <div class="flex items-center gap-2">
                <div
        ...
        <div class="flex gap-2">
          <textarea
            ...
            placeholder={hasApiKey
              ? "Ask a question about your governance..."
              : "API key required to ask questions"}
