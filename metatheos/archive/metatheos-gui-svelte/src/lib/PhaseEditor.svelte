  ...
  function validate(): boolean {
    ...
    if (!phaseId.trim()) {
      ...
      validationErrors.phaseId =
        "Phase ID can only contain letters, numbers, hyphens, and underscores";
    }
    ...
    <div class="flex-1 overflow-y-auto px-6 py-4">
      <div class="space-y-4">
        ...
        <div>
          ...
          {#if !isEditMode}
            <p class="text-gray-400 text-sm mt-1">
              Unique identifier for this phase (cannot be changed after
              creation)
            </p>
          {/if}
        </div>
        ...
        <div>
          ...
          <p class="text-gray-400 text-sm mt-1">
            Note: Use "Activate" button to set as active phase (deactivates
            other phases)
          </p>
        </div>
