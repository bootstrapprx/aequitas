<script>
  ...
  async function loadPrompts() {
    try {
      ...
      uniqueAgents = [
        ...new Set(prompts.map((p) => p.agent).filter(Boolean)),
      ].sort();
      uniquePurposes = [
        ...new Set(prompts.map((p) => p.purpose).filter(Boolean)),
      ].sort();
  ...
  function applyFilters() {
    ...
    if (agentFilter !== "all") {
      filtered = filtered.filter(
        (prompt) => prompt.agent?.toLowerCase() === agentFilter.toLowerCase(),
      );
    }
...
<div class="p-6">
  ...
  <div class="mb-6">
    ...
    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
      Curated prompts from <code
        class="bg-gray-100 dark:bg-gray-800 px-1 rounded"
        >06_PROMPTS/library</code
      >. AI transcripts (logs) are excluded.
    </p>
  </div>
  ...
  {:else if filteredPrompts.length === 0}
    <div class="text-center py-12">
      <p class="text-gray-500 dark:text-gray-400 text-lg">
        {searchQuery || agentFilter !== "all" || purposeFilter !== "all"
    ...
    <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      {#each filteredPrompts as prompt (prompt.file_path)}
        <div
          ...
          on:keydown={(e) =>
            (e.key === "Enter" || e.key === " ") && openDetailModal(prompt)}
        >
          <!-- Header -->
          <div class="flex items-start justify-between mb-3">
            <div class="flex-1 min-w-0">
              <h3
                class="font-semibold text-gray-900 dark:text-white text-lg mb-1 truncate"
              >
                {prompt.title}
              </h3>
              {#if prompt.prompt_id}
            ...
            <div class="flex items-center gap-2 flex-shrink-0">
              <span
                class="px-2 py-1 text-[10px] font-semibold rounded bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-200"
              >
                Designed Prompt
              </span>
              {#if prompt.status}
                <span
                  class="px-2 py-1 text-[10px] font-semibold rounded {getStatusBadgeClass(
                    prompt.status,
                  )}"
                >
                  {prompt.status}
          ...
          <div class="space-y-2 mb-3">
            {#if prompt.agent}
              <div class="flex items-center gap-2">
                <span
                  class="px-2 py-1 text-xs font-medium rounded {getAgentBadgeClass(
                    prompt.agent,
                  )}"
                >
                  {prompt.agent}
          ...
          {#if prompt.prompt_text}
            <p
              class="text-sm text-gray-700 dark:text-gray-300 mb-3 line-clamp-3"
            >
              {prompt.prompt_text}
            </p>
          {/if}
          ...
          <div class="flex flex-wrap gap-2 text-xs">
            {#if prompt.referenced_goals.length > 0}
              ...
              >
                🎯 {prompt.referenced_goals.length} goal{prompt.referenced_goals
                  .length !== 1
                  ? "s"
                  : ""}
              </button>
            ...
            {#if prompt.daily_references.length > 0}
              <button
                ...
                on:click|stopPropagation={() => openReferencesModal(prompt)}
              >
                📅 {prompt.daily_references.length} daily note{prompt
                  .daily_references.length !== 1
                  ? "s"
                  : ""}
              </button>
            {/if}
            {#if prompt.related_audits.length > 0}
              <button
                ...
                on:click|stopPropagation={() => openReferencesModal(prompt)}
              >
                🔍 {prompt.related_audits.length} audit{prompt.related_audits
                  .length !== 1
                  ? "s"
                  : ""}
              </button>
...
{#if showDetailModal && selectedPrompt}
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    >
      ...
      <div class="px-6 py-4 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4 text-sm">
          {#if selectedPrompt.agent}
            <div>
              ...
              <p class="mt-1">
                <span
                  class="inline-block px-2 py-1 text-xs font-medium rounded {getAgentBadgeClass(
          ...
          {#if selectedPrompt.status}
            <div>
              ...
              <p class="mt-1">
                <span
                  class="inline-block px-2 py-1 text-xs font-medium rounded {getStatusBadgeClass(
          ...
          {#if selectedPrompt.timestamp}
            <div class="col-span-2">
              ...
              >
              <p class="text-gray-900 dark:text-white mt-1">
                {formatTimestamp(selectedPrompt.timestamp)}
              </p>
            </div>
          {/if}
        </div>
        ...
        {#if selectedPrompt.prompt_text}
          <div>
            ...
            >
              <pre
                class="text-sm text-gray-900 dark:text-white whitespace-pre-wrap font-mono">{selectedPrompt.prompt_text}</pre>
            </div>
          </div>
        {/if}
              ...
              <div>
                ...
                <div class="flex flex-wrap gap-2">
                  {#each selectedPrompt.referenced_goals as goalId}
                    <span
                      class="px-2 py-1 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 rounded text-sm"
                    >
                      {goalId}
                    </span>
                  {/each}
            ...
            {#if selectedPrompt.daily_references.length > 0}
              <div>
                <p
                  class="text-sm font-medium text-gray-600 dark:text-gray-400 mb-1"
                >
                  Daily Note References ({selectedPrompt.daily_references
                    .length})
                </p>
                <div class="flex flex-wrap gap-2">
                  {#each selectedPrompt.daily_references.slice(0, 10) as date}
                    <span
                      class="px-2 py-1 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300 rounded text-sm font-mono"
                    >
                      {date}
                    </span>
                  {/each}
                  {#if selectedPrompt.daily_references.length > 10}
                    <span
                      class="px-2 py-1 text-gray-600 dark:text-gray-400 text-sm"
                    >
              ...
              <div>
                ...
                <div class="flex flex-wrap gap-2">
                  {#each selectedPrompt.related_audits as auditTitle}
                    <span
                      class="px-2 py-1 bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-300 rounded text-sm"
                    >
                      {auditTitle}
                    </span>
                  {/each}
      ...
      <div
        class="sticky bottom-0 bg-gray-50 dark:bg-gray-900 px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3"
      >
        <button
          on:click={closeDetailModal}
...
{#if showReferencesModal && selectedPromptForRefs}
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    >
            ...
            <div class="space-y-1">
              {#each selectedPromptForRefs.referenced_goals as goalId}
                <div class="px-3 py-2 bg-blue-50 dark:bg-blue-900/20 rounded">
                  <span
                    class="text-blue-700 dark:text-blue-300 font-mono text-sm"
                    >{goalId}</span
                  >
                </div>
              {/each}
            ...
            <div class="space-y-1">
              {#each selectedPromptForRefs.daily_references as date}
                <div class="px-3 py-2 bg-green-50 dark:bg-green-900/20 rounded">
                  <span
                    class="text-green-700 dark:text-green-300 font-mono text-sm"
                    >{date}</span
                  >
                </div>
              {/each}
          ...
          <div>
            ...
            <div class="space-y-1">
              {#each selectedPromptForRefs.related_audits as auditTitle}
                <div
                  class="px-3 py-2 bg-orange-50 dark:bg-orange-900/20 rounded"
                >
                  <span class="text-orange-700 dark:text-orange-300 text-sm"
                    >{auditTitle}</span
                  >
                </div>
              {/each}
      ...
      <div
        class="sticky bottom-0 bg-gray-50 dark:bg-gray-900 px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex justify-end"
      >
        <button
          on:click={closeReferencesModal}
          class="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors"
