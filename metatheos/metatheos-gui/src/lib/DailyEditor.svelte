<script>
  ...
  const statusOrder = ['active', 'planned', 'blocked', 'partial', 'done', 'archived', 'unknown']
  ...
  async function loadContext(date) {
    ...
    try {
      ...
      goalRelations = Object.fromEntries(
        (ctx.goal_relations || []).map(rel => [rel.goal.goal_id.toLowerCase(), rel])
      )
  ...
  function goalLookup() {
    const map = {}
    for (const goal of [...inPhaseGoals, ...outOfPhaseGoals]) {
      map[goal.goal_id.toLowerCase()] = goal
    }
    return map
  }

  let selectedGoalLookup = {}
  $: selectedGoalLookup = goalLookup()

  $: selectedGoalObjects = selectedGoals
    .map(id => selectedGoalLookup[id.toLowerCase()])
    .filter(Boolean)
  ...
  $: decisionOptions = dedupeDecisions([...derivedDecisions, ...recentDecisions])
  ...
  $: warnings = selectedGoalObjects
    ...
    .map(goal => `${goal.goal_id} is outside active phase ${activePhase?.phase_id || 'n/a'}`)
  ...
  function syncDerivedSections() {
    const lookup = goalLookup()
    const goalLines = selectedGoals.map(id => {
      const goal = lookup[id.toLowerCase()]
    ...
    const goalsBlock = renderLinksBlock('DERIVED_GOALS', 'Linked Goals', goalLines)
    const decisionsBlock = renderLinksBlock('DERIVED_DECISIONS', 'Linked Decisions', decisionLines)

    let nextContent = rewriteDerivedBlock(content, 'DERIVED_GOALS', goalsBlock)
    nextContent = rewriteDerivedBlock(nextContent, 'DERIVED_DECISIONS', decisionsBlock)
    content = nextContent
  }
...
<div>
  ...
  {#if devMode}
    <div class="card mb-6 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800">
      ...
      <p class="text-yellow-700 dark:text-yellow-200 text-sm">
        Run <code>cargo tauri dev</code> to hydrate the Daily Editor from the Governance Vault.
      </p>
    </div>
  {/if}
  ...
  {#if loading}
    <div class="card">
      <p class="text-gray-500 dark:text-gray-400">Loading governance context…</p>
    </div>
  {:else}
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2 space-y-4">
        ...
        <div class="card">
          <div class="flex items-center justify-between mb-2">
            <div>
              <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Goals</h3>
              <p class="text-sm text-gray-500 dark:text-gray-400">
                Select goals for {activePhase ? `Phase ${activePhase.phase_id}` : 'current day'}
              </p>
            </div>
            <div class="text-xs text-gray-500 dark:text-gray-400">
          ...
          {#if inPhaseGoals.length === 0}
            <p class="text-sm text-gray-500 dark:text-gray-400">No goals detected for the active phase.</p>
          {:else}
            {#each statusOrder as status}
              {#if groupGoals(inPhaseGoals)[status]?.length}
                <div class="mt-3">
                  ...
                  <div class="flex flex-col gap-2">
                    {#each groupGoals(inPhaseGoals)[status] as goal}
                      <button
                        class="flex items-center justify-between p-3 rounded-lg border text-left transition-colors {selectedGoals.includes(goal.goal_id)
                      ...
                      >
                        <div>
                          <div class="flex items-center gap-2">
                            <span class="font-mono text-sm font-semibold text-primary-700 dark:text-primary-300">
                              {goal.goal_id}
                            </span>
                            <span class="badge {getStatusBadge(goal.status)}">{goal.status}</span>
                          </div>
                          <div class="text-gray-900 dark:text-white font-semibold">{goal.title}</div>
                          {#if goal.dependencies?.length}
                            <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                              Depends on: {goal.dependencies.join(', ')}
                            </div>
                          {/if}
                        </div>
                        {#if selectedGoals.includes(goal.goal_id)}
                          <span class="text-sm text-primary-600 dark:text-primary-300">Linked</span>
                        {/if}
                      </button>
                    {/each}
                  </div>
          ...
          {#if outOfPhaseGoals.length > 0}
            <div class="mt-4 border-t border-dashed border-gray-200 dark:border-gray-700 pt-3">
              <div class="flex items-center gap-2 text-sm text-yellow-700 dark:text-yellow-200 mb-2">
              ...
              <div class="grid md:grid-cols-2 gap-2">
                {#each outOfPhaseGoals as goal}
                  <button
                    class="p-3 rounded-lg border text-left {selectedGoals.includes(goal.goal_id)
                      ? 'border-yellow-500 bg-yellow-50 dark:bg-yellow-900/20'
                      : 'border-gray-200 dark:border-gray-700 hover:border-yellow-400'}"
                    on:click={() => toggleGoal(goal.goal_id)}
                  >
                    <div class="flex items-center gap-2">
                      <span class="font-mono text-sm font-semibold text-yellow-800 dark:text-yellow-200">{goal.goal_id}</span>
                      <span class="badge {getStatusBadge(goal.status)}">{goal.status}</span>
                    </div>
                    <div class="text-gray-900 dark:text-white font-semibold">{goal.title}</div>
                    {#if goal.phase}
                      <div class="text-xs text-gray-500 dark:text-gray-400">Phase {goal.phase}</div>
                    {/if}
                  </button>
                {/each}
              </div>
        ...
        <div class="card">
          <div class="flex items-center justify-between mb-2">
            <div>
              <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Decisions</h3>
              <p class="text-sm text-gray-500 dark:text-gray-400">
                Link existing decisions or add a stub
              </p>
            </div>
            <button class="btn btn-secondary" on:click={addDecisionStub} disabled={devMode}>
              + Decision stub
            </button>
          </div>
          <div class="grid md:grid-cols-2 gap-3">
            <div>
              <div class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1">Linked to selected goals</div>
              {#if derivedDecisions.length === 0}
                <p class="text-sm text-gray-500 dark:text-gray-400">No linked decisions detected.</p>
              {:else}
                <div class="space-y-2">
                  {#each derivedDecisions as decision}
                    <button
                      class="w-full p-3 rounded-lg border text-left {selectedDecisions.includes(decision.decision_id)
                        ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20'
                        : 'border-gray-200 dark:border-gray-700 hover:border-primary-400'}"
                    ...
                    >
                      <div class="flex items-center justify-between">
                        <div>
                          ...
                          <div class="text-xs text-gray-500 dark:text-gray-400">{decision.decision_id}</div>
                        </div>
                        {#if decision.status}
                          <span class="badge badge-secondary text-xs">{decision.status}</span>
                        {/if}
                      </div>
                    </button>
                  {/each}
            ...
            <div>
              <div class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1">Recent (30 days)</div>
              {#if recentDecisions.length === 0}
                <p class="text-sm text-gray-500 dark:text-gray-400">No recent decisions detected.</p>
              {:else}
                <div class="space-y-2">
                  {#each recentDecisions as decision}
                    ...
                    >
                      <div class="flex items-center justify-between">
                        ...
                        <div class="text-xs text-gray-500 dark:text-gray-400">{formatDate(decision.updated || decision.date)}</div>
                      </div>
                    </button>
                  {/each}
                </div>
        ...
        <div class="card">
          <div class="flex items-center justify-between mb-2">
            <div>
              ...
              <p class="text-sm text-gray-500 dark:text-gray-400">Automatic detection plus manual notes</p>
            </div>
          </div>
          <div class="grid md:grid-cols-3 gap-3">
            <div>
              <div class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1">Goal blockers</div>
              {#if automaticGoalBlockers.length === 0}
                <p class="text-sm text-gray-500 dark:text-gray-400">None detected.</p>
              {:else}
            ...
            <div>
              <div class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1">Structural blockers</div>
              {#if structuralBlockers.length === 0}
                <p class="text-sm text-gray-500 dark:text-gray-400">No missing dependencies detected.</p>
              {:else}
                <ul class="space-y-1 text-sm text-yellow-800 dark:text-yellow-200">
            ...
            <div>
              <div class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1">External blockers</div>
              <div class="flex gap-2 mb-2">
                ...
                />
                <button class="btn btn-secondary" on:click={addManualBlocker}>Add</button>
              </div>
              {#if manualBlockers.length === 0}
                <p class="text-sm text-gray-500 dark:text-gray-400">No external blockers noted.</p>
              {:else}
                <ul class="space-y-1 text-sm text-gray-900 dark:text-white">
                  {#each manualBlockers as blocker}
                    <li class="flex items-center justify-between">
                      <span>• {blocker}</span>
                      <button class="text-xs text-red-600 dark:text-red-300" on:click={() => removeManualBlocker(blocker)}>Remove</button>
                    </li>
                  {/each}
                </ul>
              {/if}
        ...
        <div class="card">
          <div class="flex items-center justify-between mb-2">
            <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Divergences</h3>
        ...
        <div class="card">
          <div class="flex items-center justify-between mb-2">
            <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Daily body (markdown)</h3>
            <span class="text-xs text-gray-500 dark:text-gray-400">Derived links auto-inserted</span>
          </div>
          <textarea
            rows="12"
            bind:value={content}
      ...
      <div class="space-y-4">
        <div class="card">
          <div class="flex items-center justify-between mb-2">
            <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Derived Context</h3>
          ...
          <dl class="space-y-2 text-sm text-gray-900 dark:text-white">
            <div class="flex justify-between">
              <dt>Active phase</dt>
              <dd>{activePhase ? `${activePhase.phase_id} — ${activePhase.title}` : 'Unknown'}</dd>
            </div>
            <div class="flex justify-between">
              <dt>Active goals</dt>
          ...
          {#if derivedAudits.length > 0}
            <div class="mt-3">
              <div class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1">Audits touching selected goals</div>
              <ul class="space-y-1 text-sm text-gray-900 dark:text-white">
                {#each derivedAudits as audit}
                  <li>• {audit.title} {#if audit.date}<span class="text-gray-500">({formatDate(audit.date)})</span>{/if}</li>
                {/each}
              </ul>
            </div>
          {/if}
          {#if recentLinkedDecisions.length > 0}
            <div class="mt-3">
              <div class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1">Decisions affecting today’s goals</div>
              <ul class="space-y-1 text-sm text-gray-900 dark:text-white">
        ...
        <div class="card">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">Recent daily notes</h3>
          {#if dailyList.length === 0}
            <p class="text-sm text-gray-500 dark:text-gray-400">No daily notes found.</p>
          {:else}
