  ...
  function applyFilters() {
    ...
    if (riskFilter !== "all") {
      filtered = filtered.filter(
        (audit) => audit.risk?.toLowerCase() === riskFilter.toLowerCase(),
      );
    }
    ...
    <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      {#each filteredAudits as audit (audit.file_path)}
        <div
          ...
          on:keydown={(e) =>
            (e.key === "Enter" || e.key === " ") && openDetailModal(audit)}
        >
          <!-- Header -->
          <div class="flex items-start justify-between mb-3">
            <div class="flex items-center gap-2">
              <span
                class="px-2 py-1 text-[11px] font-semibold rounded bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-200"
              >
                AUDIT — Verification
              </span>
              <span
                class="px-2 py-1 text-[11px] font-semibold rounded bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200"
              >
                {audit.status || "open"}
              </span>
            </div>
            <div class="flex items-center gap-2">
              {#if audit.risk}
                <span
                  class="px-2 py-1 text-xs font-medium rounded {getRiskBadgeClass(
                    audit.risk,
                  )}"
                >
                  {audit.risk}
          ...
          <div class="flex flex-wrap gap-2 text-xs">
            {#if audit.referenced_goals.length > 0}
              ...
              >
                🎯 {audit.referenced_goals.length} goal{audit.referenced_goals
                  .length !== 1
                  ? "s"
                  : ""}
              </button>
            ...
            {#if audit.daily_references.length > 0}
              <button
                ...
                on:click|stopPropagation={() => openReferencesModal(audit)}
              >
                📅 {audit.daily_references.length} daily note{audit
            ...
            {#if audit.related_prompts.length > 0}
              <button
                ...
                on:click|stopPropagation={() => openReferencesModal(audit)}
              >
                💡 {audit.related_prompts.length} prompt{audit.related_prompts
                  .length !== 1
                  ? "s"
                  : ""}
              </button>
  ...
  >
    ...
    >
      ...
      >
        ...
        <h2
          class="text-2xl font-bold text-gray-900 dark:text-white truncate ml-3"
        >
          {selectedAudit.title}
        </h2>
        <button
        ...
        <div class="grid grid-cols-2 gap-4 text-sm">
          ...
          {#if selectedAudit.risk}
            <div>
              ...
              <p class="mt-1">
                <span
                  class="inline-block px-2 py-1 text-xs font-medium rounded {getRiskBadgeClass(
                    selectedAudit.risk,
                  )}"
                >
                  {selectedAudit.risk}
          ...
          <div class="col-span-2">
            <span class="font-medium text-gray-600 dark:text-gray-400"
              >Evidence:</span
            >
            <p class="text-gray-900 dark:text-white mt-1 truncate">
              {selectedAudit.evidence || selectedAudit.file_path}
            </p>
          </div>
        </div>
            ...
            {#if selectedAudit.daily_references.length > 0}
              <div>
                <p
                  class="text-sm font-medium text-gray-600 dark:text-gray-400 mb-1"
                >
                  Daily Note References ({selectedAudit.daily_references
                    .length})
                </p>
                <div class="flex flex-wrap gap-2">
              ...
              <div>
                ...
                <div class="flex flex-wrap gap-2">
                  {#each selectedAudit.related_prompts as promptId}
                    <span
                      class="px-2 py-1 bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300 rounded text-sm"
                    >
                      {promptId}
                    </span>
                  {/each}
          ...
          <div>
            ...
            <div class="space-y-1">
              {#each selectedAuditForRefs.related_prompts as promptId}
                <div
                  class="px-3 py-2 bg-purple-50 dark:bg-purple-900/20 rounded"
                >
                  <span
                    class="text-purple-700 dark:text-purple-300 font-mono text-sm"
                    >{promptId}</span
                  >
                </div>
              {/each}
