...
class CompanyService:
    ...
    def create_company(db: Session, company_in: CompanyCreate) -> Company:
        ...
        # Phase 5: Onboarding wizard will handle chart initialization
        # Company starts in DRAFT state (default) and must complete wizard
        # to materialize chart, setup fiscal periods, and activate accounting.
        import logging
        logger = logging.getLogger(__name__)
        logger.info(
            f"Company {db_company.ucid} created with onboarding_status={db_company.onboarding_status.value}. "
            f"User must complete onboarding wizard to activate accounting."
        )
